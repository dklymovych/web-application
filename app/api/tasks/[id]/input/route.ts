...
export async function GET(req: Request, { params }) {
  ...
  const writableStream = fs.createWriteStream('./data/graph.dat')
  const downloadStream = bucket.openDownloadStream(task['input_id'])
  ...
  const file = await readFile('./data/graph.dat', 'binary')
